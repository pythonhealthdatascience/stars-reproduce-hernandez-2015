...
base_folder <- "../python_outputs_original/results-3-obj-dohmh-data"
...
TEMP: For `combined.txt`

```{r}
results <- read.delim(file.path(base_folder, "combined.txt"))
results$forms <- results$throughput*3.2
```
...
```{r}
melted <- melt(values, id=c('resources','forms','time','prescreened', 'throughput')) %>%
  mutate(prescreen = as.numeric(gsub('[^0-9.]', '', prescreened)),
         forms = round(forms))
...
Plot only results where forms is in rrange of plots in the article.

```{r}
fig6 <- ggplot(data=melted %>% filter(forms >= 11498 & forms <= 11710)) +
  geom_bar(stat='identity',aes(x=variable, y=value, fill=prescreened)) +
  xlab("Staff Type") +
